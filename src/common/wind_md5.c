...
static void MD5Transform (w_uint32_t state[4], unsigned char block[64]); 
static void Encode (unsigned char *output, w_uint32_t *input, unsigned int len); 
static void MD5_memset (char* output,int value, unsigned int len); 
static void Decode (w_uint32_t *output, unsigned char *input, unsigned int len); 
...
void MD5Init (w_md5_ctx_s *context)          /* context */ 
{ 
    context->count[0] = 0;
    context->count[1] = 0; 
...
{ 
    ...
    if((context->count[0] += ((w_uint32_t)inputLen << 3)) 
        < ((w_uint32_t)inputLen << 3)) 
        context->count[1]++; 
    context->count[1] += ((w_uint32_t)inputLen >> 29); 
...
#endif
    //第二种用法: 
    MDString("ytsdtgcdfhjkdsuoi;fdijklfdildfhishjkdhjkdhjdfhjk",digest);//直接输入字符串并得出结果 
    system("pause");
    //第三种用法: 
