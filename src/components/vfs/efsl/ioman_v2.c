...
* allowing it to make smart decision regarding caching. The IOMAN_NUMBUFFER   *
* parameter determines how many sectors ioman can cache. ioman also supports  *
* overallocating and backtracking sectors                                     *
*                                                                             *
...
esint8 ioman_readSector(IOManager *ioman,euint32 address,euint8* buf)
{
	...
	/*wind_debug("ioman_readSector::Requesting address %u to be put in %p.\n",address,buf);*/
	if(buf==0){
		/*wind_debug("Refused to read sector, buf=0\n");*/
		return(-1);
	}
...
void ioman_printStatus(IOManager *ioman)
{
	...
	for(c=0;c<ioman->numbuf;c++){
		if(ioman_isValid(c)){
			wind_debug("BP %3i\t SC %i\t\t US %i\t RF %i\t %s %s\n",
