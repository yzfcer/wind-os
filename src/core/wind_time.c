...
static const w_uint32_t ms2frac_map[1000] = 
{
...
static w_int32_t date_to_tick32(w_uint16_t Year, w_uint16_t Month, w_uint16_t Day)
{
  w_int32_t c, ya, j;
  w_int32_t y = (w_int32_t) Year;
  w_int32_t m = (w_int32_t) Month + 1;
  w_int32_t d = (w_int32_t) Day;
...
static void datetime_to_tick64(systick_s *dst, const datetime_s *st)
{
  w_uint32_t second,fraction;
  w_int32_t jd = date_to_tick32(st->date.year,st->date.month, st->date.day);
  jd -= JAN_1ST_1900;
  second = jd;
  second = (second * 24) + st->time.hour;
  second = (second * 60) + st->time.minute;
  second = (second * 60) + st->time.second;
  fraction = ms2frac_map[st->time.msecond];
  wind_printf("sec:0x%x,frac:0x%x\r\n",second,fraction);
  dst->second = second;
  dst->frac = fraction;
}

w_uint16_t tick32_to_ms(w_uint32_t fraction)
{
...
void tick32_to_date(long JD,w_date_s *date)
{
  ...
  date->year = (w_uint16_t) y;
  date->month = (w_uint8_t) m -1;
  date->day = (w_uint8_t) d;
}

void tick64_to_datetime(datetime_s *st, systick_s *tick64)
{
  w_int32_t JD;
  w_uint32_t second,fraction;
...
#if WIND_RTC_SUPPORT
    w_chdev_s *dev;
    WIND_ASSERT_RETURN(datetime != W_NULL,W_ERR_NULL_PTR);
...
void _wind_datetime_mod_init(void)
{
    ...
    dt.week = TUS;
#endif
    datetime_to_tick64(&systick64,&dt);
    tick64_to_datetime(&dt,&systick64);
