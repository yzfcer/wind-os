...
static w_rbtree_node_s *nil = NULL; //为了避免讨论结点的边界情况，定义一个nil结点代替所有的NULL 
...
static w_rbtree_node_s* rbTreeLeft(w_rbtree_node_s *z) //返回左子树 
{ 
    return z->left; 
} 
static w_rbtree_node_s *rbTreeRight(w_rbtree_node_s *z) //返回右子树 
{ 
    return z->right; 
} 
static void LeftRotate(w_rbtree_node_s **root, w_rbtree_node_s *x) //左旋转：结点x原来的右子树y旋转成为x的父母 
{ 
    w_rbtree_node_s *y;
...
static void RightRotate(w_rbtree_node_s **root, w_rbtree_node_s *x) //右旋转：结点x原来的左子树y旋转成为x的父母 
{ 
    w_rbtree_node_s *y;
...
static void InsertFixup(w_rbtree_node_s **root, w_rbtree_node_s *z) //插入结点后, 要维持红黑树四条性质的不变性 
{ 
    w_rbtree_node_s *y; 
    if(root == NULL || z == NULL) 
        return;
...
{   
    w_rbtree_node_s *next = NULL, *refer = NULL;    /* 查找dnode的后继结点next */    
...
w_int32_t rbTreeInsert(w_rbtree_s *tree, w_rbtree_node_s *z) //插入结点 
{ 
...
w_int32_t rbTreeDelete(w_rbtree_s *tree, w_rbtree_node_s *z) //在红黑树root中删除结点z 
{    
