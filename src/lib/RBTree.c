...
static w_rbtree_node_s *nil = W_NULL; //为了避免讨论结点的边界情况，定义一个nil结点代替所有的NULL 
...
static w_rbtree_node_s* rbtree_left(w_rbtree_node_s *z) //返回左子树 
{ 
    return z->left; 
} 
static w_rbtree_node_s *rbtree_right(w_rbtree_node_s *z) //返回右子树 
{ 
    return z->right; 
} 
static void left_rotate(w_rbtree_node_s **root, w_rbtree_node_s *x) //左旋转：结点x原来的右子树y旋转成为x的父母 
{ 
    w_rbtree_node_s *y;
...
static void right_rotate(w_rbtree_node_s **root, w_rbtree_node_s *x) //右旋转：结点x原来的左子树y旋转成为x的父母 
{ 
    w_rbtree_node_s *y;
...
static void insert_fixup(w_rbtree_node_s **root, w_rbtree_node_s *z) //插入结点后, 要维持红黑树四条性质的不变性 
{ 
    w_rbtree_node_s *y; 
    if(root == W_NULL || z == W_NULL) 
        return;
    while( rbtree_parent(z)->color == TREE_RED ) //因为插入的结点是红色的，所以只可能违背性质3,即假如父结点也是红色的，要做调整 
    { 
        if( rbtree_parent(rbtree_parent(z))->left == rbtree_parent(z) ) //如果要插入的结点z是其父结点的左子树 
        { 
            y=rbtree_parent(rbtree_parent(z))->right; // y设置为z的叔父结点 
        ...
        { 
            y=rbtree_parent(rbtree_parent(z))->left; 
            if( y->color == TREE_RED) 
            { 
                z->parent->color=TREE_BLACK; 
                y->color=TREE_BLACK; 
...
static w_rbtree_node_s* successor(w_rbtree_s *tree, w_rbtree_node_s *x) //寻找结点x的中序后继 
{ 
    w_rbtree_node_s *q; 
    w_rbtree_node_s *p; 
    w_rbtree_node_s *y; 
...
{ 
    w_rbtree_node_s *parent = W_NULL, *brother = W_NULL;  
...
{   
    w_rbtree_node_s *next = W_NULL, *refer = W_NULL;    /* 查找dnode的后继结点next */    
...
* 查找特定的节点，需要外部加锁
*/
static void __rbt_middle_tranverse(w_rbtree_node_s **root,rbTreeAccess access,void *arg)
{ 
...
* 查找特定的节点，需要外部加锁
*/
w_rbtree_node_s* __rbt_search(w_rbtree_node_s **root,w_rbtree_node_s *z,rbTreeAccess access,void *arg)
{ 
...
w_int32_t rbt_insert(w_rbtree_s *tree, w_rbtree_node_s *z) //插入结点 
{ 
    w_rbtree_node_s *x; //用x保存当前顶点的父母结点，用p保存当前的结点 
    w_rbtree_node_s *p; 
    w_rbtree_node_s **root = &tree->root;
...
w_int32_t rbt_delete(w_rbtree_s *tree, w_rbtree_node_s *z) //在红黑树root中删除结点z 
{    
