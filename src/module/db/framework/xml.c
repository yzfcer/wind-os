...
{ 
    ...
    else if(is_name_tchar( *(char*)p)) { /*标记头 */ 
        p1=p;
        p2=(xstring_s*)strchr((char*)p1,'>');
        if(p2==W_NULL)
        { /*错误 */ 
...
{ 
    ...
    { 
        if( *(char*)p=='>'|| *(char*)p=='"')maxnn++;
        if( *(char*)p=='>')if( *((char*)p-1)=='/') maxnn++;/*空标记 */ 
    }
    maxnn=maxnn/2+2;
    xtree->xBuf=xmlbuf;
    ...
    *(xstr.text+xstr.len)='\0';
    ...
    { 
        ...
        case XVAL_NOTE: 
            ...
            break;
        case XVAL_NBEGIN: 
            if(xnode_add_child(xtree,&xstr)==W_NULL)k=maxnn+1; 
            break;
        case XVAL_NEND: 
...
{ 
     int k;
    xnode_s *xn;
    if(parent_node==W_NULL) return W_NULL;
