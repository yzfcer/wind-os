/****************************************Copyright (c)**************************************************
**                                       God's harbor
** FileName    : shell_framework.c
** Author      : Jason Zhou
** Last Date   : 2015/1/24 20:24:37
** Description : Shell module basic framework
**  
**--------------History---------------------------------------------------------------------------------
** Author      : Jason Zhou
** Version     : v1.0
...
w_err_t match_user_passwd(w_shell_ctx_s *ctx)
{
    ...
    if(user == W_NULL)
        return W_ERR_FAIL;
    if(wind_strcmp(ctx->passwd,user->passwd) != 0)
        return W_ERR_FAIL;
    return W_ERR_OK;
}
#else
...
#endif
static w_err_t record_user_name(w_shell_ctx_s *ctx)
{
    if(wind_strlen(ctx->buf) >= WIND_CTL_USRNAME_LEN)
    {
        wind_printf("\r\nlogin:");
        return W_ERR_FAIL;
    }
    ...
    if(match_user_name(ctx) != W_ERR_OK)
    {
        wind_memset(ctx->user,0,WIND_CTL_USRNAME_LEN);
        wind_printf("\r\nlogin:");
        return W_ERR_FAIL;
...
static void check_expected_cmd(w_shell_ctx_s *ctx)
{
    ...
    foreach_node(dnode,&g_cmdlist)
    {
        cmd = NODE_TO_CMD(dnode);
        if((wind_strncmp(cmd->name,ctx->param.argv[0],len) != 0)&&
            (wind_strstr(cmd->name,ctx->param.argv[0]) != W_NULL))
        {
            wind_printf("%-10s : ",cmd->name);
            cmd->showdisc();
        }
...
#if USER_AUTH_ENABLE
                case CSLSTAT_USER:
                    record_user_name(ctx);
                    break;
                case CSLSTAT_PWD:
                    record_user_passwd(ctx);
                    break;
#endif 
                case CSLSTAT_CMD:
