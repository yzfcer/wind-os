...
#define NODE_TO_LISTFILE(node) (w_hostfile_s*)(((w_uint8_t*)(node))-((w_uint32_t)&(((w_hostfile_s*)0)->list.listnode)))
...
static w_err_t hfs_search_child(hfile_info_s *info,char *name,w_blkdev_s *blkdev)
{
    w_err_t err;
    hfile_info_s *tmpinfo = W_NULL;
    ...
    do
    {
        ...
        tmpinfo = hostfs_mem_malloc(sizeof(hfile_info_s));
        WIND_ASSERT_BREAK(tmpinfo != W_NULL,W_ERR_MEM,"malloc tmpinfo failed");
        wind_memcpy(tmpinfo,info,sizeof(hfile_info_s));
        ...
        for(;tmpinfo != W_NULL;)
        {
            ...
            {
                wind_memcpy(info,tmpinfo,sizeof(hfile_info_s));
                break;
            }
    ...
    if(tmpinfo != W_NULL)
        hostfs_mem_free(tmpinfo);

    return err;
}

static w_err_t hfs_search_file(w_hostfs_s *lfs,w_hostfile_s *file,const char *path)
{
    w_err_t err;
    //w_uint8_t isdir = 0;
    ...
    do 
    {
        ...
        tmppath = hostfs_mem_malloc(len+1);
        ...
        //分割目录
        nameseg = (char **)hostfs_mem_malloc(LISTFS_DIR_LAYCNT * sizeof(char*));
        ...
        //分配文件信息和数据块信息结构
        finfo = hostfs_mem_malloc(sizeof(hfile_info_s));
        WIND_ASSERT_BREAK(finfo,W_ERR_MEM,"malloc finfo failed");

        //读取根目录文件信息
        err = fileinfo_read(finfo,lfs->blkdev,lfs->hfs_info.root_addr);
        WIND_ASSERT_BREAK(err == W_ERR_OK,err,"read root failed");
        if(segcnt == 1)
        {
            wind_memcpy(&file->info,finfo,sizeof(hfile_info_s));
            err = W_ERR_OK;
            break;
        }
        ...
        for(i = 1;i < segcnt;i ++)
        {
            ...
            err = hfs_search_child(finfo,nameseg[i],lfs->blkdev);
            WIND_CHECK_BREAK(err == W_ERR_OK,err);
        }
        WIND_CHECK_BREAK(err == W_ERR_OK,err);
        wind_memcpy(&file->info,finfo,sizeof(hfile_info_s));
        if(!IS_HFILE_ATTR_DIR(file->info.attr))
        {
            if(file->blkinfo == W_NULL)
            {
                file->blkinfo = hostfs_mem_malloc(sizeof(hfile_blkinfo_s));
    ...
    if(tmppath)
        hostfs_mem_free(tmppath);
    if(nameseg)
        hostfs_mem_free(nameseg);
    if(finfo)
        hostfs_mem_free(finfo);
    return err;
}

static w_err_t hfs_make_root(w_hostfs_s *lfs)
{
    w_err_t err;
    w_uint8_t attr;
    w_addr_t addr;
    ...
    hfile_blkinfo_s *blkinfo = W_NULL;
    do 
    {
        err = W_ERR_OK;
        wind_notice("hfs_make_root");
        finfo = hostfs_mem_malloc(sizeof(hfile_info_s));
        WIND_ASSERT_BREAK(finfo != W_NULL,W_ERR_MEM,"malloc finfo failed");
        blk = hostfs_mem_malloc(lfs->blkdev->blksize);
        WIND_ASSERT_BREAK(blk != W_NULL,W_ERR_MEM,"malloc blk failed.");
        blkinfo = hostfs_mem_malloc(sizeof(hfile_blkinfo_s));
        WIND_ASSERT_BREAK(blkinfo != W_NULL,W_ERR_MEM,"malloc blkinfo failed.");

        err = hostfs_bitmap_alloc_blk(&lfs->bitmap,&addr,1);
        WIND_ASSERT_BREAK(err == W_ERR_OK,err,"malloc addr failed");
        WIND_ASSERT_BREAK(addr == lfs->hfs_info.root_addr,W_ERR_FAIL,"root addr error");

        attr = (HFILE_ATTR_COMMAN | HFILE_ATTR_DIR);
        fileinfo_init(finfo,"root",lfs->hfs_info.root_addr,0,0,attr);
        blkinfo_init(blkinfo, lfs->hfs_info.root_addr,0,0,lfs->blkdev->blksize);
    ...
    if(finfo != W_NULL)
        hostfs_mem_free(finfo);
    if(blkinfo != W_NULL)
        hostfs_mem_free(blkinfo);
    return err;
}


static w_err_t hfs_make_child(w_hostfs_s *lfs,hfile_info_s *pinfo,char *name,w_uint8_t isdir)
{
    w_err_t err;
    w_uint8_t attr;
    ...
    {
        err = W_ERR_OK;
        err = hostfs_bitmap_alloc_blk(&lfs->bitmap,&self_addr,1);
        WIND_ASSERT_BREAK(err == W_ERR_OK,err,"alloc blk addr failed");
        blk = (w_uint8_t*)hostfs_mem_malloc(lfs->blkdev->blksize);
        WIND_ASSERT_BREAK(blk != W_NULL,W_ERR_MEM,"alloc blk failed");
        attr = isdir?(HFILE_ATTR_COMMAN|HFILE_ATTR_DIR):HFILE_ATTR_COMMAN;
        info = (hfile_info_s*)blk;
        ...
        wind_memcpy(pinfo,info,sizeof(hfile_info_s));
...
static w_err_t hfs_make_file(w_hostfs_s *lfs,w_hostfile_s *file,char *path)
{
    w_err_t err;
    ...
    do
    {
        ...
        tmppath = hostfs_mem_malloc(pathlen+1);
        ...
        for(i = 1;i < cnt;i ++)
        {
            ...
            if(err != W_ERR_OK)
            {
                err = hfs_make_child(lfs,finfo,nameseg[i],i == cnt - 1?isdir:1);
...
static w_err_t hostfile_destroy(w_hostfile_s* file)
{
    WIND_CHECK_RETURN(file != W_NULL,W_ERR_FAIL);
    file->info.magic = 0;
    if(file->blkinfo != W_NULL)
    {
        hostfs_mem_free(file->blkinfo);
        file->blkinfo = W_NULL;
    }
    
    if(file->subinfo!= W_NULL)
    {
        hostfs_mem_free(file->subinfo);
        file->subinfo = W_NULL;
    }
    file->lfs = W_NULL;
    hostfs_mem_free(file);
    return W_ERR_OK;
}

static w_err_t hfile_free_blkaddr(w_hostfs_s *lfs,hfile_info_s *finfo)
{
    w_err_t err;
    hfile_blkinfo_s *blkinfo = W_NULL;
    do
    {
        ...
        {
            err = W_ERR_OK;
            err = hostfs_bitmap_free_blk(&lfs->bitmap,blkinfo->dataaddr,HFILE_LBLK_CNT);
            WIND_ASSERT_BREAK(err == W_ERR_OK,err,"free blk addrs failed,can NOT be error here");
            err = hostfs_bitmap_free_blk(&lfs->bitmap,&blkinfo->self_addr,1);
...
static w_err_t do_remove_file(w_hostfs_s *lfs,hfile_info_s *finfo)
{
    w_err_t err;
    do
    {
        ...
        if(finfo->filesize > 0)
        {
            err = hfile_free_blkaddr(lfs,finfo);
            WIND_ASSERT_BREAK(err == W_ERR_OK,err,"free blkaddr failed");
            finfo->filesize = 0;
        }
        else
        {
            hostfs_bitmap_free_blk(&lfs->bitmap,&finfo->self_addr,1);
        }
        finfo->magic = 0;
    }while(0);
    return err;
...
static w_err_t hfs_get_fsinfo_by_blk(hfs_info_s *fsinfo,w_blkdev_s *blkdev,w_addr_t addr)
{
    w_err_t err;
    w_int32_t cnt;
    w_uint8_t *blk = W_NULL;
...
static w_err_t hostfs_get_fsinfo(hfs_info_s *fsinfo,w_blkdev_s *blkdev)
{
    w_int32_t i;
    w_err_t err;
    for(i = 0;i < 2;i ++)
    {
        err = hfs_get_fsinfo_by_blk(fsinfo,blkdev,i);
        if(err == W_ERR_OK)
            return W_ERR_OK;
    }
    return W_ERR_FAIL;
...
w_err_t hfs_info_read(hfs_info_s *hfs_info,w_blkdev_s *blkdev)
{
    w_err_t err;
    w_int32_t cnt;
    ...
    hfs_info_s *tmpinfo;
    w_uint8_t *blk = W_NULL;
    WIND_ASSERT_RETURN(hfs_info != W_NULL,W_ERR_PTR_NULL);
    WIND_ASSERT_RETURN(blkdev != W_NULL,W_ERR_PTR_NULL);
    do
    {
        err = W_ERR_OK;
        blk = (w_uint8_t *)hostfs_mem_malloc(blkdev->blksize);
        WIND_ASSERT_BREAK(blk != W_NULL,W_ERR_MEM,"malloc blk failed");
        ...
        tmpinfo = (hfs_info_s*)blk;
        hfs_info_be2le(tmpinfo);
        WIND_ASSERT_BREAK(tmpinfo->magic == LISTFS_MAGIC,W_ERR_INVALID,"invalid lfs head");
        wind_memcpy(hfs_info,tmpinfo,sizeof(hfs_info_s));
    }while(0);
    if(blk != W_NULL)
        hostfs_mem_free(blk);
    return err;
...
w_err_t hfs_info_write(hfs_info_s *hfs_info,w_blkdev_s *blkdev)
{
    w_err_t err;
    w_int32_t cnt;
    w_uint32_t crc;
    ...
    do 
    {
        ...
        blk = (w_uint8_t *)hostfs_mem_malloc(blkdev->blksize);
        WIND_ASSERT_BREAK(blk != W_NULL,W_ERR_MEM,"malloc blk failed");
        hfs_info_be2le(hfs_info);

        wind_memcpy(blk,hfs_info,sizeof(hfs_info_s));
...
static w_err_t hfs_info_init(w_hostfs_s *lfs,w_blkdev_s *blkdev)
{
    w_err_t err;
    hfs_info_s *hfs_info;
    ...
    hfs_info = &lfs->hfs_info;
    hfs_info->magic = LISTFS_MAGIC;
    hfs_info->blkcount = blkdev->blkcnt;
    hfs_info->unit_size = calc_unit_size(blkdev->blkcnt,blkdev->blksize);
    hfs_info->blksize = (w_uint16_t)blkdev->blksize;
    hfs_info->reserve_blk = 5;
    hfs_info->attr = 0;
    hfs_info->bitmap1_addr = hfs_info->reserve_blk + 1;
    hfs_info->bitmap_cnt = (blkdev->blkcnt - hfs_info->bitmap1_addr) / (hfs_info->unit_size + 1) + 1;
    hfs_info->bitmap2_addr = hfs_info->bitmap1_addr + hfs_info->bitmap_cnt;
    hfs_info->root_addr = hfs_info->bitmap2_addr + hfs_info->bitmap_cnt;
    
    err = hfs_info_write(hfs_info,blkdev);
...
w_err_t hostfs_format(w_hostfs_s *lfs,w_blkdev_s *blkdev)
{
    ...
    wind_notice("hostfs format,blkdev:%s",wind_obj_name(&blkdev->obj));
    do
    {
        err = W_ERR_OK;
        err = hfs_info_init(lfs,blkdev);
        WIND_ASSERT_BREAK(err == W_ERR_OK,err,"init hostfs info failed");
        err = hostfs_bitmap_init(&lfs->bitmap,lfs->hfs_info.bitmap1_addr,lfs->hfs_info.bitmap_cnt,blkdev);
        WIND_ASSERT_BREAK(err == W_ERR_OK,err,"init hostfs bitmap failed");
        err = hostfs_bitmap_clear(&lfs->bitmap);
        WIND_ASSERT_BREAK(err == W_ERR_OK,err,"clear hostfs bitmap info failed");
        err = hfs_make_root(lfs);
        WIND_ASSERT_BREAK(err == W_ERR_OK,err,"make hostfs root failed");
        lfs->file_ref = 0;
    }while(0);
    wind_notice("hostfs format result:%d",err);
    return err;
}
...
w_err_t hostfs_init(w_hostfs_s *lfs,w_blkdev_s *blkdev)
{
    ...
    do 
    {
        ...
        wind_notice("hostfs init,blkdev:%s",blkdev->obj.name);
        err = hostfs_get_fsinfo(&lfs->hfs_info,blkdev);
        WIND_ASSERT_BREAK(err == W_ERR_OK,err,"read fsinfo failed");
        
        lfs->blkdev = blkdev;
        lfs->file_ref = 0;
        hostfs_bitmap_init(&lfs->bitmap,lfs->hfs_info.bitmap1_addr,
            lfs->hfs_info.bitmap_cnt,lfs->blkdev);
        err = fileinfo_read(finfo,blkdev,lfs->hfs_info.root_addr);
...
w_err_t hostfs_deinit(w_hostfs_s *lfs)
{
    ...
    WIND_ASSERT_RETURN(lfs->hfs_info.magic == LISTFS_MAGIC,W_ERR_INVALID);
    lfs->hfs_info.magic = (~LISTFS_MAGIC);
    err = hostfs_bitmap_deinit(&lfs->bitmap);
    WIND_ASSERT_RETURN(err == W_ERR_OK,err);
...
w_err_t hostfs_match(w_blkdev_s *blkdev)
{
    ...
    do
    {
        ...
        blk = hostfs_mem_malloc(blkdev->blksize);
        WIND_ASSERT_BREAK(blk != W_NULL,W_ERR_MEM,"malloc blk failed");
        fsinfo = hostfs_mem_malloc(sizeof(hfs_info_s));
...
w_hostfile_s* hostfile_open(w_hostfs_s *lfs,const char *path,w_uint8_t mode)
{
    ...
    {
        err = W_ERR_OK;
        file = hostfs_mem_malloc(sizeof(w_hostfile_s));
        ...
        is_crt = (mode & HMODE_CRT)?W_TRUE:W_FALSE;
        
        err = hfs_search_file(lfs,file,path);
        ...
        //有创建标记，且文件不存在
        err = hfs_make_file(lfs,file,(char*)path);
...
w_err_t hostfile_close(w_hostfile_s* file)
{
    ...
    file->lfs->file_ref --;
    err = hostfile_destroy(file);
    WIND_ASSERT_RETURN(err == W_ERR_OK,err);
    return W_ERR_OK;
}

static w_err_t do_remove_dir(w_hostfs_s *lfs,hfile_info_s *finfo)
{
    w_err_t err;
    hfile_info_s *tmpinfo = W_NULL;
    do
...
w_err_t hostfile_remove(w_hostfs_s *lfs,const char *path)
{
    ...
    file = hostfile_open(lfs,path,HMODE_R);
    WIND_ASSERT_RETURN(file != W_NULL,W_ERR_PTR_NULL);
    WIND_ASSERT_RETURN(lfs->hfs_info.magic == LISTFS_MAGIC,W_ERR_INVALID);
...
w_err_t hostfile_set_attr(w_hostfile_s* file,w_uint8_t attr)
{
    ...
    WIND_ASSERT_RETURN(file != W_NULL,W_ERR_PTR_NULL);
    WIND_ASSERT_RETURN(file->info.magic == LISTFILE_MAGIC,W_ERR_INVALID);
    tmpattr = (file->info.attr & HFILE_ATTR_DIR);
    (attr & HFILE_ATTR_RDEN)?(tmpattr |= HFILE_ATTR_RDEN):(tmpattr &= ~HFILE_ATTR_RDEN);
    (attr & HFILE_ATTR_WREN)?(tmpattr |= HFILE_ATTR_WREN):(tmpattr &= ~HFILE_ATTR_WREN);
    (attr & HFILE_ATTR_HIDE)?(tmpattr |= HFILE_ATTR_HIDE):(tmpattr &= ~HFILE_ATTR_HIDE);
    (attr & HFILE_ATTR_VERIFY)?(tmpattr |= HFILE_ATTR_VERIFY):(tmpattr &= ~HFILE_ATTR_VERIFY);
...
w_err_t hostfile_seek(w_hostfile_s* file,w_int32_t offset)
{
    ...
    do 
    {
        ...
        wind_memcpy(blkinfo,file->blkinfo,sizeof(hfile_blkinfo_s));
...
static w_int32_t do_read_file(w_hostfile_s* file,w_uint8_t *buff,w_int32_t size)
{
    w_err_t err;
    w_addr_t addr; 
    w_int32_t len,blkidx,buffidx,cpsize;
    w_uint8_t *blk = W_NULL;
    if(!BLKINFO_HAS_OFFSET(file->blkinfo, file->offset, file->lfs->hfs_info.blksize))
    {
    ...
    while(buffidx < size)
    {
        ...
        if(blk == W_NULL)
        {
            blk = hostfs_mem_malloc(file->blkinfo->blksize);
            WIND_ASSERT_BREAK(blk != W_NULL, W_ERR_MEM, "malloc blk failed");
        }
...
static w_int32_t do_write_file(w_hostfile_s* file,w_uint8_t *buff,w_int32_t size)
{
    w_err_t err;
    w_addr_t addr; 
    w_int32_t len,blkidx,buffidx,cpsize;
...
static w_int32_t calc_needed_datablks(w_hostfile_s* file,w_int32_t size)
{
    w_int32_t blksize;
    w_int32_t needsize;
...
static w_int32_t calc_needed_blkinfo(w_hostfile_s* file,w_int32_t blkcnt)
{
    w_err_t err;
    w_int32_t blkinfo_cnt;
    hfile_blkinfo_s *tmpinfo = W_NULL;
    
    do
    {
        err = W_ERR_OK;
        tmpinfo = hostfs_mem_malloc(sizeof(hfile_blkinfo_s));
        WIND_ASSERT_BREAK(tmpinfo != W_NULL,W_ERR_MEM,"alloc tmpinfo failed");
        wind_memcpy(tmpinfo,file->blkinfo,sizeof(hfile_blkinfo_s));
        err = blkinfo_get_tail(tmpinfo,file->lfs->blkdev);
        WIND_ASSERT_BREAK(err == W_ERR_OK,W_ERR_FAIL,"find tail blkinfo failed.");
        if(tmpinfo->blkused + blkcnt <= HFILE_LBLK_CNT)
        {
            blkinfo_cnt = 0;
            err = W_ERR_OK;
            break;
        }
        blkcnt -= (HFILE_LBLK_CNT - tmpinfo->blkused);
        blkinfo_cnt = (blkcnt + HFILE_LBLK_CNT - 1) / HFILE_LBLK_CNT;
...
w_int32_t hostfile_read(w_hostfile_s* file,w_uint8_t *buff, w_int32_t size)
{
    ...
    WIND_ASSERT_RETURN(file != W_NULL,-1);
    WIND_ASSERT_RETURN(file->info.magic == LISTFILE_MAGIC,-1);
    WIND_ASSERT_RETURN(file->mode & HMODE_R,-1);
...
w_int32_t hostfile_write(w_hostfile_s* file,w_uint8_t *buff,w_int32_t size)
{
    ...
    WIND_ASSERT_RETURN(file != W_NULL,-1);
    WIND_ASSERT_RETURN(file->info.magic == LISTFILE_MAGIC,-1);
    WIND_ASSERT_RETURN((file->mode & HMODE_W)||(file->mode & HMODE_A),-1);
    ...
    do
    {
        ...
        {
            addrlist = (w_addr_t *)hostfs_mem_malloc((needdatablk+needblkinfo)*sizeof(w_addr_t *));
            WIND_ASSERT_BREAK(addrlist != W_NULL,W_ERR_MEM,"alloc addrlist failed");
            err = hostfs_bitmap_alloc_blk(&file->lfs->bitmap,addrlist,needdatablk+needblkinfo);
...
w_err_t hostfile_readdir(w_hostfile_s* dir,w_hostfile_s** sub)
{
    ...
    WIND_ASSERT_RETURN(dir->mode & HMODE_R,W_ERR_INVALID);
    WIND_ASSERT_RETURN(IS_HFILE_ATTR_DIR(dir->info.attr) != 0,W_ERR_INVALID);
    ...
    do
    {
        ...
        if(subhfile == W_NULL)
        {
            ...
            WIND_ASSERT_BREAK(subhfile != W_NULL,W_ERR_MEM,"malloc hostfile failed");
            wind_memcpy(&subhfile->info,&dir->info,sizeof(hfile_info_s));
            err = fileinfo_get_headchild(&subhfile->info,dir->lfs->blkdev);
        }
        else
        {
            err = fileinfo_get_next(&subhfile->info,dir->lfs->blkdev);
        }
        *sub = subhfile;
        WIND_CHECK_BREAK(err == W_ERR_OK,err);
        WIND_ASSERT_BREAK(subhfile->info.magic == LISTFILE_MAGIC,W_ERR_FAIL,"invalid fileinfo");
        
    }while(0);
    if(err != W_ERR_OK)
    {
        hostfile_destroy(subhfile);
        *sub = W_NULL;
    }
...
w_err_t hostfile_fgets(w_hostfile_s* file,char *buff, w_int32_t maxlen)
{
    ...
    len = hostfile_read(file,buff,maxlen);
